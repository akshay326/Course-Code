...
//NOTE: This code works _only_ on NxN matrix of max size 10
...
void display(m matrix){
    ...
    for (i=matrix.rs ; i<=matrix.re ; i++) {
        for (j=matrix.cs ; j<=matrix.ce ; j++)
            printf("%3d ", matrix.a[matrix.n*i + j]);
        printf("\n");
    }
    printf("\n");
}
...
m plus(m m1, m m2){
    ...
    for (m1_i=m1.rs, m2_i=m2.rs, i=0 ; m1_i<=m1.re ; m1_i++, m2_i++, i++)
        for (m1_j=m1.cs, m2_j=m2.cs, j=0 ; m1_j<=m1.ce ; m1_j++, m2_j++, j++)
            m1.a[m1.n*m1_i + m1_j] += m2.a[m2.n*m2_i + m2_j];
...
m multiply(m m1, m m2){
    ...
    if (n <= 2) {
        ...
        a = m1.a[m1.n*m1.rs + m1.cs];
        b = m1.a[m1.n*m1.rs + m1.cs+1];
        c = m1.a[m1.n*(m1.rs+1) + m1.cs];
        d = m1.a[m1.n*(m1.rs+1) + m1.cs+1];
        e = m2.a[m2.n*m2.rs + m2.cs];
        f = m2.a[m2.n*m2.rs + m2.cs+1];
        g = m2.a[m2.n*(m2.rs+1) + m2.cs];
        h = m2.a[m2.n*(m2.rs+1) + m2.cs+1];

        m3.a[m3.n*m3.rs + m3.cs]      = a*e + b*g;
        m3.a[m3.n*m3.rs + m3.cs+1]    = a*f + b*h;
        m3.a[m3.n*(m3.rs+1) + m3.cs]    = c*e + d*g;
        m3.a[m3.n*(m3.rs+1) + m3.cs+1]  = c*f + d*h;
